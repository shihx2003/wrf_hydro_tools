...
m4_name = m4name(start, end)
time_coords = pd.date_range(start, end, freq="h", tz="Asia/Shanghai").tz_convert("UTC").tz_localize(None)
m4_info = np.array(list(zip(m4_name, time_coords)))
...
for i in range(len(div_m4_info)):
    ...
    datasets = []
    results = Parallel(n_jobs=-1)(delayed(process_m4_file)(m4) for m4 in div_m4_info[i])
